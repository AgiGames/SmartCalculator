...
*/
        ...
        || ((int(expression[rightNumberPos+1]) >= 48 && int(expression[rightNumberPos+1]) <= 57) && expression[rightNumberPos-1] == '^'))
...
int main(){
    ...
    //Below code will catch errors from invalid input syntax.
    try{
        answer = inputExpression->evaluate();
    }
    catch (std::invalid_argument& e){
        std::cout << "SYNTAX ERROR, RETURNED 0\n";
    }
